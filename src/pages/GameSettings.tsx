...
function GameSettings() {
  ...
  const submitDetails = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (valid) {
      ...
      playerCtx.updateInitialPlayersData({
        players: [...players],
        balls: numOfBalls,
      });
  ...
  return (
    <>
      ...
      <form onSubmit={submitDetails}>
        <Center direction="column" style={{ overflow: "auto" }}>
          <Logo />
          ...
          <Card width="90%" margin="1rem" padding="0.5rem" direction="column">
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              <Dropdown
                ...
                menuItems={[2, 3, 4, 5]}
                action={(number: number) => setNumOfPlayers(number)}
              />
            </div>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <Text color="#919191" weight="500">
              ...
              <Dropdown
                selected={numOfBalls}
                menuItems={[1, 2, 3]}
                action={(number: number) => setNumOfBalls(number)}
              />
            </div>
            {Array.from({ length: numOfPlayers }, (_, index) => (
              <div
                style={{ display: "flex" }}
                key={index.toString() + "container"}
              >
