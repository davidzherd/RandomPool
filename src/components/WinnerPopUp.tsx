...
export const WinnerPopUp = ({ bottom, action }: Props) => {
  ...
  const handleNewRound = () => {
    ...
    const newBalls = distributeBalls(
      [...userCtx.initialPlayersData.players],
      userCtx.initialPlayersData.balls
    );
  ...
  return (
    <StyledPopUp bottom={bottom}>
      <div className="wrapper">
        <Text size={2} weight="500" shadowcolor="#DAA520">
          Congratulations!
        </Text>
        <Text size={1.5} weight="500" shadowcolor="#DAA520">
          {userCtx.playersData[0].name} is the winner
        </Text>
        <Center direction="column" margin="1rem">
          ...
          <PlayerTag
            style={{ scale: "4" }}
            background={userCtx.playersData[0].color}
          >
            <div
              style={{
                width: "25px",
          ...
          <Center direction="column" margin="5rem 0 0 0">
            ...
            <Link to="/RandomPool/new">
              <Button btncolor="gray" txtcolor="gray">
                GAME SETTINGS
              </Button>
            </Link>
          </Center>
